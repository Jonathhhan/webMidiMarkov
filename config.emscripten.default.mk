...
PLATFORM_LDFLAGS = -Wl --preload-file bin/data@data --emrun --bind -s DYNCALLS=1 -s MAX_WEBGL_VERSION=2 -s WEBGL2_BACKWARDS_COMPATIBILITY_EMULATION=1 -s USE_PTHREADS=0 -s BINARYEN_EXTRA_PASSES=--one-caller-inline-max-function-size=1 --post-js $(OF_ADDONS_PATH)/ofxEmscripten/libs/html5audio/lib/emscripten/midi.js -s USE_FREETYPE=1 -s PROXY_TO_PTHREAD=0 -s OFFSCREENCANVAS_SUPPORT=1 -s OFFSCREEN_FRAMEBUFFER=1 -s PTHREAD_POOL_SIZE=0 --profiling-funcs
...
#PLATFORM_LDFLAGS += --js-library $(OF_ADDONS_PATH)/ofxEmscripten/libs/html5audio/lib/emscripten/midi.js
...
PLATFORM_OPTIMIZATION_LDFLAGS_RELEASE = -O3 -s TOTAL_MEMORY=$(PLATFORM_EMSCRIPTEN_TOTAL_MEMORY) --memory-init-file 1 -s ERROR_ON_UNDEFINED_SYMBOLS=0
